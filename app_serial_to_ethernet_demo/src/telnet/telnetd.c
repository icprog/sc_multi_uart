...
void telnetd_recv(chanend tcp_svr, xtcp_connection_t *conn)
{
	...
	char actual_data[XTCP_CLIENT_BUF_SIZE];
	int len;
	int data_len;
	...
	parse_telnet_stream(tcp_svr, conn, hs, &data[0], &actual_data[0], len);
}
...
int telnetd_recv_data(chanend tcp_svr, xtcp_connection_t *conn, char *data, char *actual_data)
{
	...
	// Otherwise we have data, so parse it
	data_len = parse_telnet_stream(tcp_svr, conn, hs, &data[0], &actual_data[0], len);
